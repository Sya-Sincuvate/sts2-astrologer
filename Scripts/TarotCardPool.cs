...
public class TarotCardPool : TypeListCardPoolModel
{
    // 卡池的ID。必须唯一防撞车。
    public override string Title => "tarot";
    public override string EnergyColorName => "tarot";
    ...
    // 卡池的主题色。
    public override Color DeckEntryCardColor => new(0.76f, 0.856f, 1f);
    // 能量表盘文字轮廓颜色
    public override Color EnergyOutlineColor => new(0.239f, 0.286f, 0.376f);

    // 根据你使用的卡框决定使用哪个Material
    private static readonly Material? _poolFrameMaterial = MaterialUtils.CreateHsvShaderMaterial(0.6f, 0.24f, 1.0f); // 如果你使用原版卡框，使用这个直接替换色调。
